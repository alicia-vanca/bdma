...
public class SettingsPopupController {
    ...
    private void setupSegmentButtons() {
        ...
        // Changing language requires an UI reload, so no need to update button states.
        ...
        // Theme changes can be applied immediately, so update active states on click.
    ...
    private void setActiveLanguageButton() {
        applyActiveButton(AppConstants.LANG_VI.equals(I18n.getLocale().getLanguage()) ? btnVietnamese : btnEnglish,
                List.of(btnEnglish, btnVietnamese));
    }

    private void setActiveThemeButton() {
        applyActiveButton(AppConstants.THEME_DARK.equals(ThemeManager.getTheme()) ? btnDarkTheme : btnLightTheme,
    ...
    private void saveUserConfig(Theme theme, Language language) {
        try {
            ...
            // Do not save into user_config if user is not logged in (e.g. on login screen)
