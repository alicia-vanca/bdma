...
public class SyncWorker implements Runnable {
    ...
    private static final String EXTERNAL_SUFFIX =
            "/Android/data/com.bodycamera.nettysocket/cache";

    private static final List<String> TYPES =
            List.of("audio", "image", "video", "IMP", "SOS");
    ...
    public void run() {
        while (!Thread.currentThread().isInterrupted()) {
            try {
                DeviceQueue.Entry entry = queue.take();
                processDevice(entry);
    ...
    private void processDevice(DeviceQueue.Entry entry) {
    ...
    private LocalFile resolveLocalPath(FileInfo info, String remotePath) {
        ...
        File base = dataFolderManager.getDataDir();
